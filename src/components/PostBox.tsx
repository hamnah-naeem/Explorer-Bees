...
import { HiOutlinePhotograph, HiX, HiOutlineEmojiHappy } from "react-icons/hi";
...
export default function PostBox({
  ...
  const handleSubmit = () => {
    const validPollOptions =
      showPollForm && pollOptions.filter((opt) => opt.trim() !== "").length >= 2
    ...
    const postData = {
      content: value,
      image: selectedImage,
      poll: validPollOptions ? { options: validPollOptions } : null,
    };
